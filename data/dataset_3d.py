...
class ModelNet(data.Dataset):
    def __init__(self, config):
        ...
        if self.use_10k_pc and self.use_colored_pc:
            ...
            print(self.modelnet_10k_rgb_data.shape)
    ...
    def __getitem__(self, index):
        ...
        # print(current_points.shape, label, label_name)
        # torch.Size([10000, 6]) 35 toilet
...
class ShapeNet(data.Dataset):
    def __init__(self, config):
        ...
        self.picked_rotation_degrees = [
            (3 - len(str(degree))) * '0' + str(degree) if len(str(degree)) < 3 else str(degree) for degree in
            self.picked_rotation_degrees]

        # 加载 taxonomy.json 并构建 synsetId 到 name 的映射
        with open(self.id_map_addr, 'r') as f:
            self.id_map = json.load(f)

        # 生成 synsetId -> label (name) 的映射字典
        self.synset_to_label = {item['synsetId']: item['name'] for item in self.id_map}

        # 加载标签名称文件并构建 name 到行号的映射字典
        self.label_file = os.path.join(config.DATA_PATH, 'labels.txt')
        with open(self.label_file, 'r') as f:
            self.name_to_line = {line.strip(): idx for idx, line in enumerate(f)}
    ...
    def __getitem__(self, idx):
        ...
        label_name = self.synset_to_label.get(taxonomy_id, "Unknown")  # 如果没有找到对应的标签，默认为 "Unknown"

        # 获取标签的行号
        label = self.name_to_line.get(label_name, -1)  # 如果没有找到对应的标签，默认为 -1


        # 使用 tokenizer 对 label 进行分词处理
        tokenized_captions = []
        ...
        return taxonomy_id, sample['model_id'], tokenized_captions, data, label
...
class Dataset_3D():
    def __init__(self, args, tokenizer, dataset_type, train_transform=None):
        ...
        with open('./data/dataset_catalog.json', 'r') as f:
            ...
            self.dataset_split = self.dataset_catalog[self.dataset_name][dataset_type]
