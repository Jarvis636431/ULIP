...
def main(args):
    ...
    model.load_weights(weight_path = "checkpoint/initialize_models/ULIP-2-PointBERT-10k-colored-pc-pretrained.pt")
    model.cuda(args.gpu)
    ...
    criterion = models.get_cls_loss(args).cuda(args.gpu)

    p_wd, p_non_wd = [], []
    # p_wd 列表保存的是需要应用权重衰减的参数。通常是维度>= 2的权重参数，除了名字中包含 'bias'、'ln'、'bn'的参数。
    # p_non_wd 列表保存的是不需要应用权重衰减的参数。包括偏置项（名字中包含 'bias'），以及与批量归一化（Batch Normalization）或层归一化（Layer Normalization）相关的参数，或者是维度小于 2 的参数。
...
def train(train_loader, model, criterion, optimizer, scaler, epoch, lr_schedule, args):
    ...
    # model.text2pc.prompt_learner.ctx.requires_grad = False

    for name, param in model.named_parameters():
        status = "Requires Gradient" if param.requires_grad else "Frozen"
