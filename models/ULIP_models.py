...
from models.text2pc import Text2pc
...
class ULIP2_WITH_OPENCLIP(nn.Module):
    def __init__(self, point_encoder, **kwargs):
        ...
        # OpenCLIP model passed as a keyword argument
        self.open_clip_model = kwargs.open_clip_model
        # Remove unnecessary parts of the OpenCLIP model
        del self.open_clip_model.visual
        del self.open_clip_model.logit_scale

        # Initialize logit scale as a learnable parameter
        self.logit_scale = nn.Parameter(torch.ones([]) * np.log(1 / 0.07))

        # Point cloud encoder passed during initialization
        self.point_encoder = point_encoder

        # Tokenizer from OpenCLIP for processing text
        self.tokenizer = open_clip.get_tokenizer('ViT-bigG-14')

        # Initialize the projection layer for point cloud features
        self.pc_projection = nn.Parameter(torch.empty(kwargs.pc_feat_dims, 1280))
        nn.init.normal_(self.pc_projection, std=1280 ** -0.5)

        text_file = 'data/shapenet/labels.txt'

        # 实例化text2pc
        self.text2pc = Text2pc(text_file, self.open_clip_model)

        #VLPromptLearner 是传入text_file和opoencilp model，然后在text2pc中实例化VLPromptLearner,传入的class_name和

        # self.adapter = Adapter(1280, 64)

    def encode_pc(self, pc):
        pc_feat = self.point_encoder(pc)
        pc_embed = pc_feat @ self.pc_projection
        pc_embed = self.adapter(pc_embed)
        return pc_embed

    def load_weights(self, weight_path,freeze_weights=True):

        state_dict = torch.load(weight_path)

        for key, value in state_dict.items():
            print(
                f"Key: {key}, Value Type: {type(value)}, Value Shape: {value.shape if isinstance(value, torch.Tensor) else 'N/A'}")

        self.load_state_dict(state_dict, strict=False)  # strict=False to ignore missing keys
    ...
    def forward(self, pc):
        ...
        outputs,text_feat= self.text2pc(pc_embed)


        # Return embeddings without image
...
def get_pc_text_loss(args):
    return losses.ULIPWithoutImageLoss()


def get_cls_loss(args):
    return losses.ULIPClsLoss()
...
def ULIP2_PointBERT_Colored(args):
    ...
    open_clip_model, _, preprocess = open_clip.create_model_and_transforms('ViT-bigG-14', pretrained=None)
    open_clip_model.load_state_dict(
        torch.load('/home/server/tju/pointcloud/ULIP/checkpoint/open_clip_pytorch_model.bin'))
